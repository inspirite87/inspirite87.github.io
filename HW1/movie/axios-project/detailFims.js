...
document.addEventListener('DOMContentLoaded', () => {
    ...
    backButton.addEventListener('click', () => {
        window.location.href = 'axios.html'; // Измените на путь к вашему основному файлу
    });
});
...
async function loadMovieDetails(movieId) {
    try {
        ...
        if (movie.success === false) {
            throw new Error(`Error from API: ${movie.status_message}`);
        }
...
async function loadMovieImages(movieId) {
    try {
        ...
        if (data.success === false) {
            throw new Error(`Error from API: ${data.status_message}`);
        }
