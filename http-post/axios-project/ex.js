...
async function loadMovieCast(movieId) {
    try {
        ...
        if (!response.ok) throw new Error(`Failed to fetch movie cast: ${response.statusText}`);
        ...
        if (data.cast.length > 0) {
            data.cast.forEach(castMember => {
                ...
                castItem.innerHTML = `
                    <img src="${castMember.profile_path ? `https://image.tmdb.org/t/p/w500${castMember.profile_path}` : 'path/to/placeholder-image.jpg'}" alt="${castMember.name}" />
            ...
            castList.textContent = 'No cast information available.';
        }
    } catch (error) {
        console.error('Error loading movie cast:', error);
        document.getElementById('movie-cast').textContent = 'Error loading movie cast. Please try again later.';
    }
}